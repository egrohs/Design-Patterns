	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < shapes.length; ++i)
			if (shapes[i].getClass().getName().equals("LegacyLine"))
				...
				((LegacyRectangle) shapes[i]).draw(Math.min(x1, x2), Math.min(y1, y2), Math.abs(x2 - x1), Math.abs(y2
						- y1));
	}
}

// line from (10,20) to (30,60)
// retangle at (10,20) width = 20 e height = 40

// After the adapter extra level of indirection takes of mapping a user friendly
// common interface to a legacy-specific peculiar interfaces.
